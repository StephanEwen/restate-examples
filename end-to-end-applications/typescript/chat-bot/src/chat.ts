...
export const chatSessionService = restate.object({
    ...
    handlers: {
        ...
        chatMessage: async (ctx: restate.ObjectContext, message: string): Promise<ChatResponse> => {
            ...
            if (chatBroadcast) {
                chatBroadcast(ctx, ctx.key, [ `User: ${message}`, `Bot: ${JSON.stringify({
        ...
        taskDone: async (ctx: restate.ObjectContext, result: tasks.TaskResult) => {
            ...
            if (chatBroadcast) {
                chatBroadcast(ctx, ctx.key, [`Task ${result.taskName} says: ${result.result}`]);
            }
...
let chatBroadcast: ((ctx: restate.Context, session: string, msgs: string[]) => Promise<void>) | undefined = undefined;

export function broadcastHandler(handler: (ctx: restate.Context, session: string, msgs: string[]) => Promise<void>) {
    chatBroadcast = handler;
}
